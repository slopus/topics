...
The load-bearing config knob is the **durability commit class**, resolved from the box's
current config (the box `type` is immutable, but durability/config can be updated in place):
...
different `source` into one `dest` is rejected `409 box_exists_incompatible`,
...
ack never waits on the destination; the destination box's **durability class** governs only
...
filters are **per-box** and do not propagate: a delete on `source` does not reach copies

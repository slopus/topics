...
<Callout type="important">
  **Metrics are auth-gated by default when keys are configured.** Unlike `/v0/health` and
  ...
  [`read` scope](/deployment/security) when `TOPICS_API_KEYS` is set (a full-access key
...
- [Configuration](/deployment/configuration) — `TOPICS_PROBE_AUTH` and the full env var set.

...
| `wal_append_ms` | write paths | Time to enqueue the WAL frame. A `memory` box still enters the WAL write path like `disk`, so this is **not** always `0` for `memory`. (Only the pure in-memory test engine, with no WAL, reports `0`.) |
